                    ...
                    <form action="{{ route('projects.store') }}" method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <x-input id="location" name="location" type="text" class="mt-1 block w-full" value="{{ old('location') }}" />
                            ...
                            <div>
                                ...
                                <x-input id="date" name="date" type="date" class="mt-1 block w-full" value="{{ old('date') }}" />
                            ...
                            <div>
                                ...
                                <x-input id="slots" name="slots" type="number" min="0" class="mt-1 block w-full" value="{{ old('slots', 0) }}" required />
                            ...
                            <div class="border border-gray-300 dark:border-gray-700 rounded-md p-4 max-h-48 overflow-y-auto dark:bg-gray-900 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                @foreach($users as $user)
                                    <label class="flex items-center space-x-3 cursor-pointer p-2 rounded hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                                        <input type="checkbox" name="users[]" value="{{ $user->id }}" {{ is_array(old('users')) && in_array($user->id, old('users')) ? 'checked' : '' }} class="rounded dark:bg-gray-900 border-gray-300 dark:border-gray-700 text-indigo-600 shadow-sm focus:ring-indigo-500 dark:focus:ring-indigo-600">
