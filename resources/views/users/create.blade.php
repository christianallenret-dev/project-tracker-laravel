    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <form action="{{ route('users.store') }}" method="POST" class="p-6 lg:p-8 space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <div class="mt-4 p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg border border-gray-200 dark:border-gray-700">
                            <label class="flex items-start space-x-3 cursor-pointer">
                                <input type="checkbox" id="is_manager" name="is_manager" value="1" {{ old('is_manager') ? 'checked' : '' }} class="rounded dark:bg-gray-900 border-gray-300 dark:border-gray-700 text-indigo-600 shadow-sm focus:ring-indigo-500 mt-0.5">
                                <div>
                    ...
                    <div class="pt-6">
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400 mb-4">{{ __('Optionally assign this user to a project with a role and date range.') }}</p>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select id="project_role" name="project_role" class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm mt-1 block w-full">
                                    ...
                                    <option value="developer" {{ old('project_role') == 'developer' ? 'selected' : '' }}>{{ __('Developer') }}</option>
                                    <option value="manager" {{ old('project_role') == 'manager' ? 'selected' : '' }}>{{ __('Manager') }}</option>
                                    <option value="qa" {{ old('project_role') == 'qa' ? 'selected' : '' }}>{{ __('QA') }}</option>
                                </select>
                                <x-input-error for="project_role" class="mt-2" />
                            </div>
                            <div>
                                <x-label for="assigned_at" value="{{ __('Start Date') }}" />
                                <x-input id="assigned_at" name="assigned_at" type="date" class="mt-1 block w-full" value="{{ old('assigned_at') }}" />
                                <x-input-error for="assigned_at" class="mt-2" />
                            </div>
                            <div>
                                <x-label for="ended_at" value="{{ __('End Date') }}" />
                                <x-input id="ended_at" name="ended_at" type="date" class="mt-1 block w-full" value="{{ old('ended_at') }}" />
